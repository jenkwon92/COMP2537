...
const updatePaginationDiv = (currentPage, numPages) => {
	...
	$('#pagination').append(`
    <button class="btn btn-primary prevButton" ${currentPage === 1 ? 'style="display:none;"' : ''}>Prev</button>
  `);
	...
	// Create Next button
	$('#pagination').append(`
    <button class="btn btn-primary nextButton" ${currentPage === numPages ? 'style="display:none;"' : ''}>Next</button>
  `);
};
...
const paginate = async (currentPage, PAGE_SIZE, pokemons) => {
	...
	console.log(pokemons.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE))
...
const filterByTypes = (types) => {
	...
	if(filteredPokemons.length == 0){	
		var nextBtn  = document.getElementsByClassName('nextButton')[0];
...
const setup = async () => {
	...
	updatePokemonCounts(PAGE_SIZE, pokemons.length);
